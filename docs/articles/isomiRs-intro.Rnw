...
\bioctitle[isomiRs]{isomiRs: miRNAoma analysis from small-RNAseq data}
\author{Lorena Pantano$^{1*}$, Georgia Escaramis$^{2*}$,
Eulalia Martin$^{2*}$ \\ \small{Harvard TH Chan School of Public Health,
Boston, US;} \\ \small{$^{2}$ Center of Genomic Regulation, Barcelona,
Spain;} \\ \small{\texttt{$^*$lpantano (at) gmail.com}}}
...
\packageVersion{\Sexpr{BiocStyle::pkg_ver("isomiRs")}}
...
information used for visualization and statistical analysis. It is a subclass of \Rclass{SummarizedExperiment} with \Rfunction{colData} and \Rfunction{counts} methods. 
...
The user can access the normalized count matrix with \Rcode{counts(object, norm=TRUE)}.
...
\Rcode{metadata(mirData)} contains two lists: \Rfunction{rawList} is a list with same
length than number of samples and stores the input files
for each sample; \Rfunction{isoList} is a list with same length than
...
the data stored in \Rfunction{isoList} for sample 1 and 5' changes
with this code \Rcode{metadata(ids)[["isoList"]][[1]]["t5sum"]}.
...
brain samples \cite{pantano2016} to give some basic examples of isomiRs analyses.

In this data set we will find two groups:

\begin{itemize}
    \item pc: 7 control individuals
    \item pt: 7 patients with Parkinson's Disease in early stage.
\end{itemize}
...
The \Rcode{type} parameter controls what type of isomiRs to show. It can be
...
\Rcode{counts(ids)} and \Rcode{colData(ids)} like this:
...
\Rpackage{DiscriMiner} into \Rfunction{isoPLSDA} function. The output
