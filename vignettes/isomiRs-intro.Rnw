...
information used for visualization and statistical analysis. It is a subclass of \Rfunction{SummarizedExperiment} with \Rfunction{colData} and \Rfunction{counts} methods. 
...
\Rfunction{metadata(mirData)} contains two lists: \Rfunction{rawList} is a list with same
...
with this code \Rfunction{metadata(ids)[["isoList"]][[1]]["t5sum"]}.
