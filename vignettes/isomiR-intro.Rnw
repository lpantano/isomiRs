...
The input should be the output of SeqBuster-miraligner file (.mirna) 
for each sample in the following format:

<<format-line,eval=FALSE,echo=TRUE>>==
seq     name    freq    mir     start   end     mism    add     t5      t3      s5      s3      DB      ambiguity
TGTAAACATCCTACACTCAGCTGT        seq_100014_x23  23      hsa-miR-30b-5p  17      40      0       0       0       d-GT    TTCATGTA        AGCTGTAA        miRNA   1
TGTAAACATCCCTGACTGGAA   seq_100019_x4   4       hsa-miR-30d-5p  6       26      13TC    0       0       u-G     TTGTTGTA        GAAGCTGT        miRNA   2
TGTAAACATCCCTGACTGGAA   seq_100019_x4   4       hsa-miR-30e-5p  17      37      12CT    0       0       u-G     CTACTGTA        GAAGCTGT        miRNA   2
CAAATTCGTATCTAGGGGATT   seq_100049_x1   1       hsa-miR-10a-3p  63      81      0       u-TT    0       u-ATA   GTCACAAA        AATATGTA        miRNA   1
TGACCTAGGAATTGACAGCCAGT seq_100060_x1   1       hsa-miR-192-5p  25      47      8GT     0       d-C     d-AGT   GGCTCTGA        AGCCAGTG        miRNA   1
@
...
raw and normalized counts from miraligner allowing to update the summarization of miRNAs.
...
IsomiRs names follows this structure: 

\begin{itemize}
    \item miRNA name
    \item type: ref if the sequence is the same than the miRNA reference. `iso` if the sequence has variations.
    \item t5 tag: indicates variations at 5' position. The naming contains two words: `direction - nucleotides`, where direction can be `u` (upstream of the 5' position reference) or `d` (downstream of the 5' position reference). `0` indicates no variation, then the 5' position is the same than the reference. After `direction`, it follows the nucleotide/s that are added (in the case of the upstream type) or deleted (in the case of the downstream type).
    \item t3 tag: indicates variations at 3' position. The naming contains two words: `direction - nucleotides`, where direction can be `u` (upstream of the 3' position reference) or `d` (downstream of the 3' position reference). `0` indicates no variation, then the 3' position is the same than the reference. After `direction`, it follows the nucleotide/s that are added (in the case of the downstream type) or deleted (in the case of the upstream type).
    \item ad tag: indicates nucleotides additions at 3' position. The naming contains two words: `direction - nucleotides`, where direction is `u` (upstream of the 5' position reference). `0` indicates no variation, then the 3' position has no additions. After `direction`, it follows the nucleotide/s that are added.
    ...
    \item seed tag: same than `mm` tag, but only if the change happens from nucleotide 2 to 8.
\end{itemize}
...
a mRNA differential expression analysis.
...
This function has parameters to create a matrix using only the referenceces
...
\Rfunction{isoCounts} can get the counts matrix that can be use for many
...
allows quick integration to another analyses like clustering or PCA.
...
components (t1, t2, t3 ...) from the PLS-DA analysis and the 
