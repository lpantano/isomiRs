...
\title{isomiRs }
\author{Lorena Pantano$^{1*}$, Georgia Escaramis$^{2*}$, Eulalia Martin$^{2*}$ \\ \small{Harvard H Chan School of Public Health, Boston, US;} \\ \small{$^{2}$ Center of Genomic Regulation, Barcelona, Spain;} \\ \small{\texttt{$^*$lpantano (at) iscb.org}}}
...
the \Rfunction{countsIso} can get the matrix counts that can be use for many different downstream analyses configuring the way isomiRs are collapsed. Similar to the previous point.
