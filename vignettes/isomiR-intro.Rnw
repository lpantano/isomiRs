...
The funciont \Rfunction{loadIso} needs a vector with the paths for each file
...
You can differenciate between reference sequences, and isomirs at 5' end
doing this:
...
different downstream analyses configuring the way isomiRs are collapsed.
